...
export default function Select({ handleChange }) {
    const selectEl = Object.entries(data).map(([ key, value ]) => (
        <div key={key} className="flex flex-col gap-2 text-gray-300">
            ...
            <select
                className='bg-slate-900 text-gray-300 p-2 border border-slate-900 rounded-md cursor-pointer hover:bg-gray-400 hover:text-slate-900 focus:bg-gray-400 focus:text-slate-900 focus:shadow-sm'
                name={key}
                id={key}
